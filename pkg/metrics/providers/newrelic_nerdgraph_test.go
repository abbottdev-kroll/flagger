...
func TestNewNerdGraphProvider(t *testing.T) {
	...
	t.Run("default host", func(t *testing.T) {
		...
		assert.Equal(t, int64(60), provider.fromDelta)
	})
	...
	t.Run("invalid account id", func(t *testing.T) {
		cs := map[string][]byte{
			"newrelic_api_key":    []byte(apiKey),
			"newrelic_account_id": []byte("not-a-number"),
		}
		_, err := NewNerdGraphProvider("1m", flaggerv1.MetricTemplateProvider{}, cs)
		require.Error(t, err)
	...
	t.Run("invalid metric interval", func(t *testing.T) {
		cs := map[string][]byte{
			"newrelic_api_key":    []byte(apiKey),
			"newrelic_account_id": []byte(accountID),
		}
		_, err := NewNerdGraphProvider("invalid", flaggerv1.MetricTemplateProvider{}, cs)
		require.Error(t, err)
...
func TestNerdGraphProvider_RunQuery(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			assert.Contains(t, reqBody.Query, expectedGQLSubstring)
			assert.Contains(t, reqBody.Query, "query ($query: Nrql!)")

			finalNRQL, ok := reqBody.Variables["query"].(string)
			require.True(t, ok)
			assert.Contains(t, finalNRQL, nrqlQuery)
			assert.Contains(t, finalNRQL, "SINCE 60 SECONDS ago")
